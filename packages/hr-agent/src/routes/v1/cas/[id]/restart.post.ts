...
export default async function restartCARoute(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const taskId = await manager.run(
      ...
      CONTAINER_TASK_PRIORITIES.RESTART
    );
    ...
    res.json(
      new Result({
        ...
        status: 'pending_restart',
        taskId,
        message: 'Container restart task queued'
      })
    );
  } catch (error) {
