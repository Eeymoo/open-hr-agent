/* eslint-disable max-nested-callbacks */
import { describe, it, expect, vi, beforeEach } from 'vitest';
import { ConnectCaTask } from './connectCaTask.js';
import { EventBus } from '../services/eventBus.js';
import { TaskLogger } from '../utils/taskLogger.js';
import { TASK_STATUS } from '../config/taskStatus.js';
import { TASK_EVENTS } from '../config/taskEvents.js';
import { TASK_TAGS } from '../config/taskTags.js';

vi.mock('../utils/docker/getContainer.js', () => ({
  getContainerByName: vi.fn()
}));

vi.mock('../utils/database.js', () => ({
  getPrismaClient: vi.fn(() => ({
    task: {
      update: vi.fn().mockResolvedValue({})
    }
  })),
  getCurrentTimestamp: vi.fn(() => Date.now())
}));

describe('ConnectCaTask', () => {
  let task: ConnectCaTask;
  let eventBus: EventBus;
  let logger: TaskLogger;
  let mockGetContainerByName: ReturnType<typeof vi.fn>;

  beforeEach(async () => {
    eventBus = new EventBus();
    logger = new TaskLogger();
    vi.spyOn(logger, 'info').mockResolvedValue(undefined);
    vi.spyOn(logger, 'warn').mockResolvedValue(undefined);
    vi.spyOn(logger, 'error').mockResolvedValue(undefined);

    task = new ConnectCaTask(eventBus, logger);

    const { getContainerByName } = await import('../utils/docker/getContainer.js');
    mockGetContainerByName = vi.mocked(getContainerByName);
    vi.useFakeTimers();
  });

  describe('execute', () => {
    describe('有 parentTaskId 的情况', () => {
      it('应该更新主任务状态为 CONNECTING_CA', async () => {
        mockGetContainerByName.mockResolvedValue({
          id: 'container-123',
          name: 'hra_123',
          state: 'running',
          status: 'Up 1 hour'
        });

        const result = await task.execute(
          { parentTaskId: 99, caName: 'hra_123', issueNumber: 123 },
          { taskId: 1, taskName: 'connect_ca', retryCount: 0 }
        );

        expect(result.success).toBe(true);
      });

      it('应该在 nextParams 中传递 parentTaskId', async () => {
        mockGetContainerByName.mockResolvedValue({
          id: 'container-123',
          name: 'hra_123',
          state: 'running',
          status: 'Up 1 hour'
        });

        const result = await task.execute(
          { parentTaskId: 99, caName: 'hra_123', issueNumber: 123 },
          { taskId: 1, taskName: 'connect_ca', retryCount: 0 }
        );

        expect(result.nextParams?.parentTaskId).toBe(99);
      });
    });

    describe('容器就绪检查', () => {
      it('应该等待容器状态变为运行中', async () => {
        mockGetContainerByName.mockResolvedValue({
          id: 'container-123',
          name: 'hra_123',
          state: 'running',
          status: 'Up 1 hour'
        });

        const result = await task.execute(
          { caName: 'hra_123', issueNumber: 123 },
          { taskId: 1, taskName: 'connect_ca', retryCount: 0 }
        );

        expect(result.success).toBe(true);
      });

      it('应该处理容器状态异常', async () => {
        mockGetContainerByName.mockResolvedValue({
          id: 'container-123',
          name: 'hra_123',
          state: undefined,
          status: 'Exited (1) 1 minute ago'
        });

        const result = await task.execute(
          { caName: 'hra_123', issueNumber: 123 },
          { taskId: 1, taskName: 'connect_ca', retryCount: 0 }
        );

        expect(result.success).toBe(false);
        expect(result.error).toContain('CA 容器 hra_123 状态异常');
      });
    });

    describe('返回值', () => {
      it('应该返回 finalStatus 为 COMPLETED', async () => {
        mockGetContainerByName.mockResolvedValue({
          id: 'container-123',
          name: 'hra_123',
          state: 'running',
          status: 'Up 1 hour'
        });

        const result = await task.execute(
          { caName: 'hra_123', issueNumber: 123 },
          { taskId: 1, taskName: 'connect_ca', retryCount: 0 }
        );

        expect(result.finalStatus).toBe(TASK_STATUS.COMPLETED);
      });

      it('应该返回正确的 nextTask 为 ai_coding', async () => {
        mockGetContainerByName.mockResolvedValue({
          id: 'container-123',
          name: 'hra_123',
          state: 'running',
          status: 'Up 1 hour'
        });

        const result = await task.execute(
          { caName: 'hra_123', issueNumber: 123 },
          { taskId: 1, taskName: 'connect_ca', retryCount: 0 }
        );

        expect(result.nextTask).toBe('ai_coding');
        expect(result.nextEvent).toBe(TASK_EVENTS.CA_CONNECTED);
      });
    });
  });

  describe('任务属性', () => {
    it('应该有正确的 name 属性', () => {
      expect(task.name).toBe('connect_ca');
    });

    it('应该有正确的 dependencies', () => {
      expect(task.dependencies).toEqual(['create_ca']);
    });

    it('应该有正确的 tags', () => {
      expect(task.tags).toContain(TASK_TAGS.MANAGES_CA);
      expect(task.tags).toContain(TASK_TAGS.SUBTASK);
    });
  });
});
